...
# Tema 3: Creación de presentaciones de PPTX con       #
#         officer pt2                                  #
# Sesión: 04                                           #
...
# 4) Creación del PPTX con Officer -----

## 4.1) Crear documento -----

doc <- read_pptx("template1.pptx")
...
layout_properties(doc, layout = "Diapositiva de título")

slide_size(doc)

## 4.2) Textos -----

txt_navy_bold <- function(s, size = 18) fp_text(color = navy, bold = TRUE, font.size = size)
txt_gray <- function(s, size = 14) fp_text(color =gray_tx, font.size = size)

## 4.3) Slide 1: Portada (layout "Diapositiva de título") ----

doc <- add_slide(
  doc, layout = "Diapositiva de título", master = "Marco"
)
...
doc <- ph_with(
  ...
  value = fpar(ftext("Análsis de desempeño comercial | Datos simulados con fines pedagógicos",
                     fp_text(color = navy, italic = TRUE, font.size = 16))),
  location = ph_location_type(type = "subTitle")
)

## 4.4) Slide 2: Separador de sección -----

doc <- add_slide(doc, layout = "Encabezado de sección", master = "Marco")
...
doc <- add_slide(doc, layout = "Título y objetos", master = "Marco")
...
doc <- ph_with(doc, external_img("img/chart_trimestre.png",
               ...
               location = ph_location(left = 4, top = 1.9,
                                      width = 8.5, height = 4.3))
...
doc <- add_slide(doc, layout = "Solo el título", master = "Marco")
doc <- ph_with(
  doc,
...
print(doc, target = "Reporte_Ventas_2026_template.pptx")
